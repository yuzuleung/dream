...
function ChartTooltipContent(props: any) {
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    if (labelFormatter) {
      return <div className={cn("font-medium", labelClassName)}>{labelFormatter(value, _payload)}</div>;
    }

    if (!value) return null;
    return <div className={cn("font-medium", labelClassName)}>{value}</div>;
  }, [label, labelFormatter, _payload, hideLabel, labelClassName, config, labelKey]);
                ...
                <>
                  ...
                  <div className={cn("flex flex-1 justify-between leading-none", nestLabel ? "items-end" : "items-center")}>
                    ...
                    {item.value !== undefined && (
                      <span className="text-foreground font-mono font-medium tabular-nums">{(item.value as number).toLocaleString?.() ?? String(item.value)}</span>
                    )}
                  </div>
                </>
              )}
